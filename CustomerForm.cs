...
namespace inventory_management_system
{
    public partial class CustomerForm : Form
    {
        ...
        public void LoadCustomer()
        {
            ...
            cn = new SqlCommand("SELECT * FROM tbCustomer", con);
            con.Open();
            dr = cn.ExecuteReader();
            while (dr.Read())
            {
        ...
        private void dgvCustomer_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (colName == "Edit")
            {
                ...
                customerModule.IbICId.Text = dgvCustomer.Rows[e.RowIndex].Cells[1].Value.ToString();
            ...
            else if (colName == "Delete")
            {
                ...
                {
                    con.Open();
                    cn = new SqlCommand("DELETE FROM tbCustomer WHERE cid LIKE '" + dgvCustomer.Rows[e.RowIndex].Cells[1].Value.ToString() + "'", con);
                    cn.ExecuteNonQuery();
                    con.Close();
